...
Eigen::MatrixXd KDLHelper::getTransform(std::string base_frame, std::string tip_frame)
{
    Eigen::MatrixXd T;
    T = Eigen::MatrixXd::Identity(4,4);
    //std::cout << "[KDL] getTransform() base: " << base_frame << " tip: " << tip_frame << std::endl;
    ...
    _tree.getChain(base_frame,tip_frame,chain);
    int segmentN = chain.getNrOfSegments()-1;
    // std::cout << "[KDL] getTransform() segments: " << segmentN << std::endl;
    
    frame = chain.getSegment(segmentN).getFrameToTip();
    ...
    T(1,3) = t.y();
    T(2,3) = t.z();
    // std::cout << "[KDL] getTransform() trasl: " << t.x() << " " << t.y() << " " << t.z() << std::endl;
